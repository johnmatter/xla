...
#ifndef XLA_HE_PJRT_CLIENT_H
#define XLA_HE_PJRT_CLIENT_H
...
class HePjRtBuffer : public PjRtBuffer {
 public:
  HePjRtBuffer(HePjRtClient* client, std::unique_ptr<PjRtBuffer> wrapped);
  ~HePjRtBuffer() override;
...
// creates.
class HePjRtClient : public PjRtClient {
 public:
  static std::unique_ptr<HePjRtClient> CreateHePjRtClient(
      std::unique_ptr<PjRtClient> wrapped);
  explicit HePjRtClient(std::unique_ptr<PjRtClient> wrapped);
  ~HePjRtClient() override;
  ...
  StatusOr<PjRtDevice*> LookupAddressableDevice(
    ...
    if (wrapped_ == nullptr) {
      return tsl::errors::Internal(
          "Wrapped PJRT client in HePjRtClient is already destoryed.");
    }
  ...
  StatusOr<std::unique_ptr<PjRtBuffer>> CreateUninitializedBuffer(
    ...
    return Unimplemented(
        "CreateUninitializedBuffer not supported for HePjRtClient.");
  }
  StatusOr<std::unique_ptr<AsyncHostToDeviceTransferManager>>
  CreateBuffersForAsyncHostToDevice(absl::Span<const Shape> shapes,
                                    PjRtDevice* device) override {
    return Unimplemented(
        "AsyncHostToDeviceTransferManager not supported for HePjRtClient.");
  }
  StatusOr<std::unique_ptr<AsyncHostToDeviceTransferManager>>
  CreateBuffersForAsyncHostToDevice(absl::Span<const Shape> shapes,
  ...
  // Tracks a non-owning pointer of HePjRtBuffer in HePjRtClient.
  void TrackBuffer(HePjRtBuffer* buffer);

  // Untracks a HePjRtBuffer in HePjRtClient.
  void UntrackBuffer(const HePjRtBuffer* buffer);

  // Destroy all the wrapped PjRtBuffer in the tracked set of HePjRtBuffers and
  // then destroy the wrapped PjRtClient.
  void DestroyWrappedBuffersAndClient();

 private:
  // Unwraps a HePjRtBuffer.
  PjRtBuffer* UnwrapBuffer(PjRtBuffer* buffer) const {
    return tensorflow::down_cast<HePjRtBuffer*>(buffer)->wrapped();
  }

  // Unwraps a HePjRtExecutable.
  const PjRtLoadedExecutable& UnwrapExecutable(
      const PjRtLoadedExecutable& executable) const {
    return *tensorflow::down_cast<const HePjRtExecutable*>(&executable)
                ->wrapped();
  }
  ...
  // Depending on `sizeof(absl::flat_hash_set<HePjRtBuffer*>)`, might need to
  // add some padding to the struct.
  struct DeviceBuffers {
    absl::Mutex mu;
    absl::flat_hash_set<HePjRtBuffer*> alive_buffers ABSL_GUARDED_BY(mu);
  };
